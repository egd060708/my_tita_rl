...
class HeadingLeggedRobot(BaseTask):
    ...
    def _init_buffers(self):
        ...
        self.foot_positions = self.rigid_body_states[:, self.feet_indices,
                              0:3]
        self.cur_footpos_translated = self.foot_positions - self.root_states[:, 0:3].unsqueeze(1)
        self.footpos_in_body_frame = torch.zeros(self.num_envs, 2, 3, device=self.device)
        for i in range(2):
            self.footpos_in_body_frame[:, i, :] = quat_rotate_inverse(self.base_quat, self.cur_footpos_translated[:, i, :])
        ...
        self.commands_scale = torch.tensor([self.obs_scales.lin_vel, self.obs_scales.lin_vel, self.obs_scales.ang_vel, self.obs_scales.lin_vel], device=self.device, requires_grad=False,) # TODO change this
        ...
        self.single_contact_side = torch.zeros(self.num_envs, len(self.feet_indices), dtype=torch.float, device=self.device, requires_grad=False)
        self.single_contact_side_last = torch.zeros(self.num_envs, len(self.feet_indices), dtype=torch.float, device=self.device, requires_grad=False)
        self.single_contact = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
        self.single_feet_air_time = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
        self.shift_check_idx = torch.zeros(self.num_envs, dtype=torch.int64, device=self.device, requires_grad=False)
        self.shift_check_idx_last = torch.zeros(self.num_envs, dtype=torch.int64, device=self.device, requires_grad=False)
        self.check_shift = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
        self.check_shift_en = torch.zeros(self.num_envs, dtype=torch.int64, device=self.device, requires_grad=False)
        self.check_shift_en_last = torch.zeros(self.num_envs, dtype=torch.int64, device=self.device, requires_grad=False)
        self.en_shift = torch.zeros(self.num_envs, dtype=torch.int64, device=self.device, requires_grad=False)
        
        self.LegP = torch.zeros(self.num_envs, 2, 2, dtype=torch.float, device=self.device, requires_grad=False)
        self.LegV = torch.zeros(self.num_envs, 2, 2, dtype=torch.float, device=self.device, requires_grad=False)
        self.LegL = torch.zeros(self.num_envs, 2, dtype=torch.float, device=self.device, requires_grad=False)
        self.LegA = torch.zeros(self.num_envs, 2, dtype=torch.float, device=self.device, requires_grad=False)
        self.LegdA = torch.zeros(self.num_envs, 2, dtype=torch.float, device=self.device, requires_grad=False)
        self._update_custom_physics()
    ...
    def _create_envs(self):
        ...
        # get penalized contacts and save as lists
        ...
        # get init base state from config files
        base_init_state_list = self.cfg.init_state.pos + self.cfg.init_state.rot + self.cfg.init_state.lin_vel + self.cfg.init_state.ang_vel
        self.base_init_state = to_torch(base_init_state_list, device=self.device, requires_grad=False)
        start_pose = gymapi.Transform()
        start_pose.p = gymapi.Vec3(*self.base_init_state[:3])

        # create maps in simulation
    ...
    def reindex(self,tensor):
        ...
        Args:
            tensor (tensor): the tensor to be reindexed

        Returns:
            tensor: reindexed tensor
        """
        #sim2real purpose
    ...
    def keyboard_cmd(self):
        """keyboard check for simple play
        """
        self.commands[:, 0] = 0.
        self.commands[:, 1] = 0.
        self.commands[:, 2] = 0.
        ...
        if self.viewer:
            for evt in self.gym.query_viewer_action_events(self.viewer):
                if evt.action == "vx_plus":
                    if evt.value > 0:
                        self.my_key_pressed[0] = True
                    elif evt.value == 0:
                        self.my_key_pressed[0] = False
                if evt.action == "vx_minus":
                    if evt.value > 0:
                        self.my_key_pressed[1] = True
                    elif evt.value == 0:
                        self.my_key_pressed[1] = False
                if evt.action == "vy_plus":
                    if evt.value > 0:
                        self.my_key_pressed[2] = True
                    elif evt.value == 0:
                        self.my_key_pressed[2] = False
                if evt.action == "vy_minus":
                    if evt.value > 0:
                        self.my_key_pressed[3] = True
                    elif evt.value == 0:
                        self.my_key_pressed[3] = False
                if evt.action == "left_turn":
                    if evt.value > 0:
                        self.my_key_pressed[4] = True
                    elif evt.value == 0:
                        self.my_key_pressed[4] = False
                if evt.action == "right_turn":
                    if evt.value > 0:
                        self.my_key_pressed[5] = True
                    elif evt.value == 0:
                        self.my_key_pressed[5] = False
        if self.my_key_pressed[0] == True:
            self.commands[:, 0] = 1.
        elif self.my_key_pressed[1] == True:
            self.commands[:, 0] = -1.
        else:
            self.commands[:, 0] = 0.
        if self.my_key_pressed[2] == True:
            self.commands[:, 1] = 1.
        elif self.my_key_pressed[3] == True:
            self.commands[:, 1] = -1.
        else:
            self.commands[:, 1] = 0.
            
        if self.my_key_pressed[4] == True:
            self.keyboard_heading = self.keyboard_heading + 0.01
        elif self.my_key_pressed[5] == True:
            self.keyboard_heading = self.keyboard_heading - 0.01
        self.commands[:, 3] = self.keyboard_heading
    ...
    def post_physics_step(self):
        ...
        self.foot_positions = self.rigid_body_states[:, self.feet_indices,
                              0:3]
        self.cur_footpos_translated = self.foot_positions - self.root_states[:, 0:3].unsqueeze(1)
        # print("rootState: ",self.root_states[:, 0:3].unsqueeze(1))
        # print("footPos: ", cur_footpos_translated)
        self.footpos_in_body_frame = torch.zeros(self.num_envs, 2, 3, device=self.device)
        for i in range(2):
            self.footpos_in_body_frame[:, i, :] = quat_rotate_inverse(self.base_quat, self.cur_footpos_translated[:, i, :])
        ...
        self.single_contact = torch.logical_xor(self.contact_filt[:,0],self.contact_filt[:,1])
        # left contact 1, right contact 2, other 0
        # self.single_contact_side = ((self.contact_filt[:,0]>self.contact_filt[:,1])+1)*self.single_contact
        
        self.single_contact_side[:,0] = self.contact_filt[:,0]>self.contact_filt[:,1]
        self.single_contact_side[:,1] = self.contact_filt[:,0]<self.contact_filt[:,1]
        shift_check_idx_expanded = self.shift_check_idx.unsqueeze(1).to(torch.long)
        nowside = torch.gather(self.single_contact_side, dim=1, index=shift_check_idx_expanded).squeeze()
        lastside = torch.gather(self.single_contact_side_last, dim=1, index=shift_check_idx_expanded).squeeze()
        self.shift_check_idx = torch.logical_xor(self.shift_check_idx,(nowside>lastside))
        # get real shift support
        self.check_shift = torch.logical_and(torch.logical_xor(self.shift_check_idx,self.shift_check_idx_last),self.check_shift_en)
        self.check_shift = torch.logical_xor(self.shift_check_idx,self.shift_check_idx_last)
        self.shift_check_idx_last = self.shift_check_idx.clone()
        self.single_contact_side_last = self.single_contact_side.clone()
        
        # once single contact, init shift_check_idx
        self.check_shift_en = torch.logical_or(self.check_shift_en,self.single_contact)
        self.en_shift = torch.logical_xor(self.check_shift_en, self.check_shift_en_last)
        self.check_shift_en_last = self.check_shift_en.clone()
        self.shift_check_idx = torch.logical_or(self.shift_check_idx, self.en_shift*(self.contact_filt[:,0]>self.contact_filt[:,1]))
        self.shift_check_idx_last = torch.logical_or(self.shift_check_idx_last, self.en_shift*(self.contact_filt[:,0]>self.contact_filt[:,1]))
    ...
    def _update_custom_physics(self):
        self.LegP[:,0,1] = -0.2*torch.cos(self.dof_pos[:,1])-0.2*torch.cos(self.dof_pos[:,1]+self.dof_pos[:,2])
        self.LegP[:,0,0] = -0.2*torch.sin(self.dof_pos[:,1])-0.2*torch.sin(self.dof_pos[:,1]+self.dof_pos[:,2])
        self.LegP[:,1,1] = -0.2*torch.cos(self.dof_pos[:,5])-0.2*torch.cos(self.dof_pos[:,5]+self.dof_pos[:,6])
        self.LegP[:,1,0] = -0.2*torch.sin(self.dof_pos[:,5])-0.2*torch.sin(self.dof_pos[:,5]+self.dof_pos[:,6])
        self.LegV[:,0,1] = 0.2*torch.sin(self.dof_pos[:,1])*self.dof_vel[:,1] + \
                            0.2*torch.sin(self.dof_pos[:,1]+self.dof_pos[:,2])*(self.dof_vel[:,1]+self.dof_vel[:,2])
        self.LegV[:,0,0] = -0.2*torch.cos(self.dof_pos[:,1])*self.dof_vel[:,1] - \
                            0.2*torch.cos(self.dof_pos[:,1]+self.dof_pos[:,2])*(self.dof_vel[:,1]+self.dof_vel[:,2])
        self.LegV[:,1,1] = 0.2*torch.sin(self.dof_pos[:,5])*self.dof_vel[:,5] + \
                            0.2*torch.sin(self.dof_pos[:,5]+self.dof_pos[:,6])*(self.dof_vel[:,5]+self.dof_vel[:,6])
        self.LegV[:,1,0] = -0.2*torch.cos(self.dof_pos[:,5])*self.dof_vel[:,5] - \
                            0.2*torch.cos(self.dof_pos[:,5]+self.dof_pos[:,6])*(self.dof_vel[:,5]+self.dof_vel[:,6])
        self.LegA[:,0] = torch.atan2(-self.LegP[:,0,0],-self.LegP[:,0,1])
        self.LegA[:,1] = torch.atan2(-self.LegP[:,1,0],-self.LegP[:,1,1])
        self.LegL[:,0] = torch.norm(self.LegP[:,0],dim=-1)
        self.LegL[:,1] = torch.norm(self.LegP[:,1],dim=-1)
        direction = torch.zeros(self.num_envs,2,2,dtype=torch.float,device=self.device,requires_grad=False)
        direction[:,0,0] = torch.cos(-self.LegA[:,0])
        direction[:,0,1] = torch.sin(-self.LegA[:,0])
        direction[:,1,0] = torch.cos(-self.LegA[:,1])
        direction[:,1,1] = torch.sin(-self.LegA[:,1])
        self.LegdA[:,0] = -(torch.sum(self.LegV[:,0]*direction[:,0],dim=-1))/self.LegL[:,0]
        self.LegdA[:,1] = -(torch.sum(self.LegV[:,1]*direction[:,1],dim=-1))/self.LegL[:,1]
    ...
    def _compute_torques(self, actions):
        ...
        torques[:,[3, 7]] = self.kp_factor[:,[3, 7]]  * self.cfg.control.wheel_stiffness *(joint_pos_target[:,[3, 7]]) - self.cfg.control.wheel_damping * self.kd_factor[:,[3, 7]] *(self.dof_vel[:,[3, 7]])
    ...
    def reset_idx(self, env_ids):
        ...
        self.single_contact_side[env_ids] = 0. # 0 represent both contact, 1 left contact, 2 right contact
        self.single_contact[env_ids] = 0.
        self.single_feet_air_time[env_ids] = 0.
        self.shift_check_idx[env_ids] = 0
        self.shift_check_idx_last[env_ids] = 0
        self.check_shift[env_ids] = 0
        self.check_shift_en[env_ids] = 0
        self.check_shift_en_last[env_ids] = 0
        self.en_shift[env_ids] = 0
    ...
    def _reset_root_states(self, env_ids):
        ...
        if self.cfg.domain_rand.randomize_init_pitch == True:
            angle_xyz[:,1] = torch_rand_float(self.cfg.domain_rand.init_pitch_range[0],self.cfg.domain_rand.init_pitch_range[1],(len(env_ids), 1),device=self.device).view(len(env_ids))
    ...
    def _reward_leg_vel(self):
        # Penalize leg velocities
        return torch.sum(torch.square(self.LegdA), dim=1)
    ...
    def _reward_tracking_heading(self):
        # Tracking of heading angle (yaw)
        heading_cur = torch.atan2(self.projected_forward[:,1],self.projected_forward[:,0])
        heading_err = torch.square(wrap_to_pi(self.commands[:, 3] - heading_cur))
        return torch.exp(-heading_err/self.cfg.rewards.head_tracking_sigma)
    
    def _reward_tracking_yaw(self):
        # Tracking of yaw angle commands
        real_yaw_error = torch.zeros([self.num_envs, 1, 1], dtype=torch.float, device=self.device, requires_grad=False)
        real_yaw_error = self.commands[:, 3] - self.yaw
        for i in range(self.num_envs):
            while real_yaw_error[i] >= torch.pi:
                real_yaw_error[i] = real_yaw_error - 2*torch.pi
            while real_yaw_error[i] < -torch.pi:
                real_yaw_error[i] = real_yaw_error[i] + 2*torch.pi
        real_yaw_error = torch.square(real_yaw_error)
        return torch.exp(-real_yaw_error/self.cfg.rewards.tracking_sigma)
    ...
    def _reward_feet_air_time(self):
        ...
        rew_airTime = torch.sum((self.feet_air_time - self.cfg.rewards.ref_feet_fly_time) * first_contact, dim=1) # reward only on first contact with the ground
    ...
    def _reward_foot_relative_x(self):
        ...
        for i in range(2):
            footpos_in_body_frame[:, i, :] = quat_rotate_inverse(self.base_quat, self.cur_footpos_translated[:, i, :])
    ...
    #     wheel_veoutputl_vel_err[:,i] = self.base_lin_vel[:, 0] - wheel_vel[:, i]*0.0948
    ...
    def _reward_feet_y_distance(self):
        # Penalize if feet stay too close or too far
        # use dim=-1 to divide envs
        feet_distance = torch.abs(self.footpos_in_body_frame[:, 1, 1] - self.footpos_in_body_frame[:, 0, 1])
        reward = torch.clip(self.cfg.rewards.min_feet_distance - feet_distance, 0, 1) + \
                 torch.clip(feet_distance - self.cfg.rewards.max_feet_distance, 0, 1)
        return reward
    
    def _reward_feet_x_constrain(self):
        
        return 0
    
    def _reward_avoid_fork(self):
        
        return 0
    
    def _reward_survival(self):
        # Reward if survival for some time
        return (self.episode_length_buf * self.dt) > 5
    
    def _reward_ref_feet_point_2contact(self):
        # Mannually set points for feet to track while 2 contact
        ref_footpoint_left = torch.tensor(self.cfg.rewards.ref_footpoint_left,device=self.device)
        ref_footpoint_right = torch.tensor(self.cfg.rewards.ref_footpoint_right,device=self.device)
        footpos_in_body_frame_ref = torch.zeros(self.num_envs, 2, 3, device=self.device)
        footpos_in_body_frame_ref[:,0] = ref_footpoint_left
        footpos_in_body_frame_ref[:,1] = ref_footpoint_right
        reward = torch.clip(torch.norm(footpos_in_body_frame_ref[:,0,:2] - self.footpos_in_body_frame[:,0,:2],dim=-1), 0, 1) +\
                torch.clip(torch.norm(footpos_in_body_frame_ref[:,1,:2] - self.footpos_in_body_frame[:,1,:2],dim=-1), 0, 1)
        return reward
    
    def _reward_balance_margain(self):
        # Penalize if unsatisfied balance margain range
        left_margin = torch.abs(torch.norm(self.footpos_in_body_frame[:,0,1])/torch.mean(self.root_states[:, 2].unsqueeze(1) - self.measured_heights, dim=1))
        right_margin = torch.abs(torch.norm(self.footpos_in_body_frame[:,1,1])/torch.mean(self.root_states[:, 2].unsqueeze(1) - self.measured_heights, dim=1))
        left_reward = torch.clip(left_margin,2*(self.cfg.rewards.balance_margin_y_min-self.cfg.rewards.balance_margin_y_mid),0.5*(self.cfg.rewards.balance_margin_y_max-self.cfg.rewards.balance_margin_y_mid))
        right_reward = torch.clip(right_margin,2*(self.cfg.rewards.balance_margin_y_min-self.cfg.rewards.balance_margin_y_mid),0.5*(self.cfg.rewards.balance_margin_y_max-self.cfg.rewards.balance_margin_y_mid))
        return torch.log(1 + left_reward + right_reward)
    
    def _reward_single_contact(self):
        return self.single_contact
    
    def _reward_ref_feet_point_1contact(self):
        # reward = self.single_contact*torch.sum(self.contact_filt*torch.clip(torch.norm(0 - self.footpos_in_body_frame[:,:,:2],dim=-1), 0, 1),dim=-1)
        # 直接计算世界坐标系下的位置
        reward = self.single_contact*torch.sum(self.contact_filt*torch.clip(torch.norm(self.cur_footpos_translated[:,:,:2],dim=-1), 0, 1),dim=-1)
        return reward
    
    def _reward_long_single_contact(self):
        self.single_feet_air_time += self.dt
        rew_airTime = self.single_feet_air_time - self.cfg.rewards.ref_feet_fly_time
        # self.feet_air_time *= torch.logical_or(self.single_contact,torch.logical_not(torch.logical_or(self.contact_filt[:,0],self.contact_filt[:,1])))
        self.single_feet_air_time *= self.single_contact
        # self.single_feet_air_time *= ~self.check_shift
        # self.single_feet_air_time *= self.check_shift_en
        return rew_airTime
    
    def _reward_stay_one_side(self):
        return self.check_shift
    
    # def _reward_ref_feet_point_1contact(self):
    #     # Mannually set points for feet to track while 1 contact
    #     # zero_point = torch.zeros(self.num_envs, device=self.device)
    #     reward = torch.zeros(self.num_envs,device=self.device)
    #     for i in range(self.num_envs):
    #         if self.contact_filt[i,0] != self.contact_filt[i,1]:
    #             if self.contact_filt[i,0] == True:
    #                 reward[i] = torch.clip(torch.norm(0 - self.footpos_in_body_frame[i,0,:2],dim=-1), 0, 1)
    #             else:
    #                 reward[i] = torch.clip(torch.norm(0 - self.footpos_in_body_frame[i,1,:2],dim=-1), 0, 1)
    #         else:
    #             reward[i] = 0
    #     return reward
    
    # def _reward_single_contact(self):
    #     # use in single leg support
    #     # Reward if use single leg support
    #     # should not big scale
    #     reward = torch.zeros(self.num_envs,device=self.device)
    #     for i in range(self.num_envs):
    #         if self.contact_filt[i,0] == self.contact_filt[i,1]:
    #             reward[i] = 0
    ...
    # def _reward_long_single_contact(self):
    #     # Reward if use long term single leg support
    #     # can be big scale
    #     for i in range(self.num_envs):
    #         self.single_contact_time[i] = self.single_contact_time[i] + self.dt
    #         # while single contact state fail, time is reset
    #         if self.single_contact_side[i] == 1. and self.contact_filt[i,1] == True:
    #             self.single_contact_time[i] = 0.
    #         if self.single_contact_side[i] == 2. and self.contact_filt[i,0] == True:
    #             self.single_contact_time[i] = 0.
    #         # judge if single contact
    #         if self.contact_filt[i,0] == True and self.contact_filt[i,1] == True:
    #             self.single_contact_side[i] = 0.
    #             self.single_contact_time[i] = 0.
    #         else:
    #             if self.contact_filt[i,0] == True:
    #                 self.single_contact_side[i] = 1.
    #             else:
    #                 self.single_contact_side[i] = 2.

    #     return torch.clip(torch.square(self.single_contact_time),min=0,max=4)
    
    
    #------------ cost functions----------------
    def _cost_legAngle_limit(self):
        ul = torch.tensor(self.cfg.rewards.legA_limit[1],dtype=torch.float,device=self.device).repeat((self.num_envs,1))
        ll = torch.tensor(self.cfg.rewards.legA_limit[0],dtype=torch.float,device=self.device).repeat((self.num_envs,1))
        upper = 1.*(self.LegA[:,0] > ul) + 1.*(self.LegA[:,1] > ul)
        lower = 1.*(self.LegA[:,0] < ll) + 1.*(self.LegA[:,1] < ll)
        out_limit = 1.*(torch.sum(upper + lower,dim=1) > 0.0)
        return out_limit
    
    def _cost_legAngleV_limit(self):
        return 1.*(torch.sum(1.*(torch.abs(self.LegdA) > self.cfg.rewards.legAV_limit*self.cfg.rewards.soft_legAV_limit),dim=1) > 0.0)
